...
@Component({
  ...
  template: `
    <div class="min-h-screen flex bg-[#FFFBF5]">
      ...
      >
        <div class="h-full bg-[#1A1A1A] text-white rounded-3xl p-4 flex flex-col justify-between shadow-2xl border border-white/10 relative overflow-hidden backdrop-blur-xl">
          <!-- Background Subtle Gradient Overlay -->
          <div class="absolute -top-24 -right-24 w-48 h-48 rounded-full bg-[#7743DB]/30 blur-3xl pointer-events-none"></div>
          ...
          <div class="relative z-10">
            <div class="flex items-center justify-between gap-3 pb-6 border-b border-white/10">
              <a routerLink="/" class="flex items-center gap-3 overflow-hidden">
                <div class="w-10 h-10 rounded-2xl bg-gradient-to-tr from-[#7743DB] via-[#9055EE] to-[#C3ACD0] p-0.5 shadow-lg flex-shrink-0">
                ...
                @if (!sidebarCollapsed()) {
                  <div class="overflow-hidden">
                    <h2 class="font-extrabold text-base tracking-tight leading-none text-white">Zero<span class="text-[#C3ACD0]">Hunger</span></h2>
                    <span class="text-[10px] text-[#5B5B6A] uppercase font-bold tracking-wider">SaaS Platform</span>
                  </div>
                }
              </a>
            ...
            @if (!sidebarCollapsed() && currentUser(); as user) {
              <div class="mt-4 p-3 rounded-2xl bg-white/5 border border-white/10 flex items-center gap-3">
                <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-[#7743DB] to-[#C3ACD0] text-white flex items-center justify-center font-bold text-sm shadow-md">
                  {{ user.fullName.charAt(0).toUpperCase() }}
                </div>
                <div class="overflow-hidden flex-1">
                  <p class="text-xs font-bold text-white truncate">{{ user.fullName }}</p>
                  <span class="text-[10px] uppercase tracking-wider font-semibold text-[#C3ACD0] block">{{ user.role }}</span>
                </div>
              </div>
            }
            ...
            <nav class="mt-6 space-y-1.5 overflow-y-auto max-h-[calc(100vh-280px)] pr-1">
              @for (item of filteredNavItems(); track item.path) {
                <a
                  ...
                  routerLinkActive="bg-[#7743DB] text-white font-bold shadow-lg shadow-[#7743DB]/30 border-white/20"
                ...
                >
                  ...
                  @if (!sidebarCollapsed()) {
                    ...
                    @if (item.badge) {
                      <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-[#EF4444] text-white">{{ item.badge }}</span>
                    }
                  }
                </a>
              }
      ...
      >
        ...
        <header class="sticky top-0 z-40 h-20 px-8 flex items-center justify-between backdrop-blur-xl bg-[#FFFBF5]/80 border-b border-[#E8DDD3]">
          <!-- Global Search Pill -->
          <div class="relative max-w-md w-full">
            <div class="flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-[#F7EFE5] border border-[#E8DDD3] focus-within:border-[#7743DB] focus-within:bg-white transition-all">
              <svg class="w-4 h-4 text-[#5B5B6A]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
              </svg>
              <input
                type="text"
                placeholder="Search food, donations, logistics... (⌘K)"
                class="w-full bg-transparent border-none text-xs text-[#1A1A1A] placeholder-[#5B5B6A] focus:outline-none"
              />
            </div>
          </div>
          ...
          <div class="flex items-center gap-4">
            <a
              ...
              class="btn-primary py-2.5 px-4 text-xs font-semibold rounded-2xl flex items-center gap-2 shadow-md shadow-[#7743DB]/20"
            >
            ...
            <div class="relative">
              <button
                ...
                class="flex items-center gap-3 p-1.5 pr-3 rounded-2xl border border-[#E8DDD3] bg-[#F7EFE5] hover:border-[#7743DB]/30 transition-all"
              >
                <div class="w-8 h-8 rounded-xl bg-[#7743DB] text-white flex items-center justify-center font-bold text-xs">
                  {{ currentUser()?.fullName?.charAt(0) || 'U' }}
                </div>
                <span class="text-xs font-bold text-[#1A1A1A] hidden sm:inline">{{ currentUser()?.fullName }}</span>
                <svg class="w-4 h-4 text-[#5B5B6A]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
              </button>

              @if (showProfileMenu()) {
                <div class="absolute right-0 mt-3 w-60 glass-panel p-2 rounded-3xl shadow-2xl z-50 border border-[#E8DDD3]" @dropdown>
                  <div class="px-3 py-2 border-b border-[#E8DDD3] mb-1">
                    <p class="font-bold text-xs text-[#1A1A1A]">{{ currentUser()?.fullName }}</p>
                    <p class="text-[11px] text-[#5B5B6A] truncate">{{ currentUser()?.email }}</p>
                  </div>
                  <a routerLink="/dashboard/settings" (click)="showProfileMenu.set(false)" class="flex items-center gap-2.5 px-3 py-2.5 text-xs font-medium text-[#1A1A1A] hover:bg-[#F7EFE5] rounded-xl transition-colors">
                    <svg class="w-4 h-4 text-[#7743DB]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                    Settings & Profile
                  </a>
                  <button (click)="logout()" class="w-full text-left flex items-center gap-2.5 px-3 py-2.5 text-xs font-semibold text-[#EF4444] hover:bg-rose-50 rounded-xl transition-colors mt-1">
                    <svg class="w-4 h-4 text-[#EF4444]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
                    Sign Out
                  </button>
                </div>
              }
